...
### 2.8.2 에서 추가된 주요 기능들
- 성경 읽기 기능에서 멀티 버퍼링을 구현해서 좀 더 원활하게 읽어주도록 개선하였습니다. 다만 느린 기기에서는 처음 읽기 시작할때 약간 기다리셔야 할 수도 있습니다.  
- 성경 읽기 기능에서 이제 특정 구절까지만 읽도록 설정이 가능합니다.  
...
- 성경 본문에 있는 RF, Rf 태그로 둘러쌓인 노트 부분을 지금까지 처리하지 않고 있었는데, 이제 윗첨자로 표기하고 윗첨자를 누르면 노트 내용이 새창에서 뜨도록 하였습니다.

### 2.7.1 에서 추가된 주요 기능들
- 초소형 AI TTS 엔진으로 성경을 낭독해주는 기능.  
- 음성 낭독시 타이머 기능  
- 백그라운드에서도(꺼진화면에서도) 음성 재생 지원.  

### 2.6.3 에서 추가된 주요 기능들
- 아이패드에서 주석창의 크기가 전체 화면의 95%를 차지하도록 크기 변경.  
- 리딩플랜 CSV 파일들내의 오류 수정  

### 2.6.2 에서 추가된 주요 기능들
- 앱 설치시 복사되어야 할 리딩플랜 CSV 파일들이 복사되지 않던 문제 해결.  

### 2.6.1 에서 추가된 주요 기능들
- 이미지를 추가했을때 이미지 비율이 보존되지 않은채 보여지던 문제 해결.  
- iCloud 동기화를 켰을때에도 이미지가 로컬에 저장되던 문제 해결.  
