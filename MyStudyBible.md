...
그리고 대부분의 데이터들을 직접 구해서 앱을 설치하면 생성되는 MyStudyBible 폴더 아래의 여러 폴더들에 직접 복사해 넣어줘야 하는 불편함도 있습니다. 그러나 라이센스 문제를 회피하기위해서 어쩔수 없이 이런 선택을 해야만 했습니다.

참고로 아래의 네이버카페에 가시면 해당 데이터들을 조금 더 구하실수 있을겁니다. 
...
- 아이클라우드 동기화가 켜져있으면, 아이폰과 아이패드에서 동일한 데이터로 작업을 이어서 하는 것이 가능합니다. 물론 지역에 따라 동기화의 속도가 느릴수 있으므로 10여분 정도 기다려야 하는 일이 있을수 있습니다.
